...
"""
@author:Chenyu Wang
@file: cent_abstract.py
@time: 2019/03/13
@v1.0  基于规则中心的文本摘要算法，主要基于TF-IDF，关键词，句子所在位置，句子长度信息。
"""
...
import autils as autils


class Centr(object):

    def __init__(self):
        self.stop_file='lib/stopwords_all.txt'
        print("Initialize Centroid model finished!")

    # Converting Sentences to Vectors
    # 向量化过程使用的是词袋模型(BOW)

    def convertToVSM(self,sentences):
        vocabulary = []
    ...
    输出
    key_val:以句子数为长度的list，其中每个单元即为keyword在该句中出现的次数*10/关键词总出现次数
    '''

    def calcKey_val(self,sentences,stopwords,title=None):
        key_val=np.zeros(len(sentences)).tolist()
        if title :
            title_cut = dc.clean_cut(title)
            title_cut = dc.scan_stopwords(title_cut,stopwords)
            for index,line in enumerate(sentences):
                vec = []
                for tokentit in title_cut.split(' '):
                    vec.append(line.count(tokentit))
                key_val[index] = sum(vec)
            key_val = np.array(key_val) / sum(key_val) * 10
            return self.softmax(key_val.tolist())
        else:
            return key_val
    ...
    def calcMeanTF_ISF(self,VSM):
        tfvec = []
        isfvec = []
        tfisfvec = []
        tfisfvec_rbm = []
        sentences_len=len(VSM)
        for index in range(sentences_len):
            ...
            for i in range(vocab_len):
                ...
                if (tf > 0):
                    ...
                    tfisf += (tf) * log(sentences_len / sent_freq)
            if (count > 0):
            ...
            tfvec.append(tf)
            isfvec.append(log(sentences_len/sent_freq))
            tfisfvec.append(mean_tfisf * 100)
            tfisfvec_rbm.append(mean_tfisf)

        return tfvec, isfvec, self.softmax(tfisfvec_rbm)

    '''
    输入
    sentences：list of sentence [line1,line2,line3...]
    bias: 句子词数倾向，如果1 则为均值，
    处理逻辑：
    不希望选择太长或者太短的句子，因此选择句子词数的均值作为标准
    输出结果范围：0-1
    '''
    def calcsentlen(self,sentences,bias=1):
        # # Sentence length feature
        from math import log10
        sent_word = []
        for u in range(len(sentences)):
            sent_split = sentences[u].split(" ")
            sent_split = [w for w in sent_split if not w.isdigit()]
            a = (len(sent_split))
            sent_word.append(a)

        ##OR BY THIS METHOD: LENGTH OF SENTENCE/ LONGEST SENTENCE
        longest_sent = max(sent_word)
        mean_sent = np.mean(sent_word)
        sent_length = []
        sent_length_rbm = []
        for x in sent_word:
            sent_length.append((1-abs(mean_sent-x)/50) * 100)
            sent_length_rbm.append(1-abs(mean_sent-x)/50)
        # print(sent_length)

        return sent_length,self.softmax(sent_length_rbm),sent_word

    '''
    计算句子所在的位置，并设定首句及尾句的权重最高，其他句离首句越远则得分越低
    '''
    def calcpos(self,sentences):
        sent_position = [i for i in range(len(sentences))]
        ...
        return self.softmax(position_rbm)

    def write2file(self, res,filename):
        with open(filename,'w') as f:
            f.write(res+'\n')

    def return_index(self,score_list,sum_size):
        ind = np.argpartition(score_list,-sum_size)[-sum_size:]
        return ind

    '''
    将TF-IDF，关键词，句子长度，句子位置信息输入后加权求和，并排序得到总得分最高的句子作为输出
    '''
    def calcTotal(self,raw,tf_isf,key_val,sens_lens,sens_pos,sum_size=1,wisf=10,wkey=2,wlens=4,wpos=2):
            score = wisf*np.array(tf_isf)+wkey*np.array(key_val)+wlens*np.array(sens_lens)+wpos*np.array(sens_pos)
            if sum_size < len(score):
                index_list = self.return_index(score,sum_size)
                tpl = []
                for i in index_list:
                    tpl.append((raw[i],int(i)))
                # print("################################")
                # print(tpl)
                res,index_list = autils.sort_tuple_list(tpl)
    ...
    def single_content(self,data,sum_size):
        ...
        t0=time()
        cut_con = dg.contcut(data)
        raw_con = dg.cont(data)
        t1 = time()
        # print('Using Centroid, data clean:{}s'.format(round(t1-t0,2)))

        VSM = self.convertToVSM(cut_con)
        t2 = time()
        # print('sentence vector gene:{}s'.format(round(t2-t1,2)))

        tfvec, isfvec, tfisfvec_rbm = self.calcMeanTF_ISF(VSM)
        t3 = time()
        # print('tf-isf:{}s'.format(round(t3-t2,2)))

        key_val = self.calcKey_val(cut_con, self.stop_file)
        t4 = time()
        # print('key_words:{}s'.format(round(t4-t3,2)))

        _, sent_length_rbm, sent_word = self.calcsentlen(cut_con)
        t5 = time()
        # print('length weight:{}s'.format(round(t5-t4,2)))

        sent_pos2_rbm = self.calcpos(cut_con)
        t6 = time()
        # print('postion weight:{}s'.format(round(t6-t5,2)))

        sum_list,index_list = self.calcTotal(raw_con,tfisfvec_rbm,key_val,sent_length_rbm,sent_pos2_rbm,sum_size)
        t7 = time()
        # print('return summary list:{}s'.format(round(t7-t6,2)))

        return sum_list,index_list


    def cent_live_proc(self,input_dict):
...
if __name__=='__main__':
    ...
    cont_dict['content']='''十三届全国人大二次会议在人民大会堂开幕'''
    '''
    经济观察网讯 3月5日上午9时，十三届全国人大二次会议在人民大会堂开幕，听取国务院总理李克强关于政府工作的报告，审查国务院关于2018年国民经济和社会发展计划执行情况与2019年国民经济和社会发展计划草案的报告，审查国务院关于2018年中央和地方预算执行情况与2019年中央和地方预算草案的报告。
    ...
    time1=time()
    centest = Centr()
    test=centest.cent_live_proc(cont_dict)
